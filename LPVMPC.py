...
class LPVOcp(AcadosOcp):
    def __init__(self, N, Tf):
        ...
        self.prev_x = np.array([[0, 0, 1, 0, 0, 0, 0]])
    ...
    def set_dynamics(self) -> None:
        ...
        A = A(self.model.p[:self.n_states], self.model.p[self.n_states:self.n_states+self.n_params], 
              self.model.p[self.n_states+self.n_params:self.n_states+self.n_params+self.n_inputs])
        B = cs.jacobian(f, u)
        B = cs.Function("B", [x, u, p], [B])
        B = B(self.model.p[:self.n_states], self.model.p[self.n_states:self.n_states+self.n_params], 
              self.model.p[self.n_states+self.n_params:self.n_states+self.n_params+self.n_inputs])
        print(B)
        print(A)
    ...
    def set_cost(self) -> None:
        ...
        self.parameter_values = np.concatenate((self.prev_x[:, 0], np.array([9.0]), self.prev_u[:, 0]))
    ...
    def optimize(self, x0, waypoints, p, lin_mode:str = "prev_iter"):
        ...
        # Use previous iteration as lin point
            for i in range(self.N):
                ...
                self.solver.set(i, "p", np.array([
                                                self.prev_x[0, i+1], # x
                                                self.prev_x[1, i+1], # y
                                                self.prev_x[2, i+1], # cos heading
                                                self.prev_x[3, i+1], # sin heading 
        ...
        self.prev_x = np.array([
            ...
            trajectory[:, 4], # v_y
            trajectory[:, 5], # r
            trajectory[:, 6], # steering angle
        ])
        self.prev_u = np.array([
            inputs[:, 0],     # steering rate
        ])
        print(self.prev_u.shape)
        print(self.prev_x.shape)
        print(inputs[:15])
